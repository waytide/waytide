...
An experiment is kept: it's a dated file under `waytide/local/experiments/`, committed with the code, not a scratch note you throw away.
...
*The experiment conventions come from the `foundation` package (the record format, and the branch-and-lifecycle rules). Installing `foundation` gives you the `waytide/local/experiments/` directory and these conventions. See [Getting started](#) for installation.*
