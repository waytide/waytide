...
2. **~~`Constant`-example generalization~~ — DONE (2026-07-17).** The `testing` and `code/ruby` rules now illustrate with the neutral **`Upload`** domain (a useful object: dependency, `build` factory, `call` actuator) and its value object **`Upload::Result`** (build-normalizes/new-strict) — one example matching the Doctrine of Useful Objects. Both packages re-published. **On hold:** the build-constructor rule's supertype-as-factory-of-subtypes sub-point still uses `Constant` (no `Upload` subtypes yet); return to it later.
