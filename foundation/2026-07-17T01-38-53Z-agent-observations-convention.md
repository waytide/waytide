# `agent/observations/` — working hypotheses, not yet binding
...
- **Promotion:** when an observation stabilizes into a binding convention, lift it into `agent/rules/` as a new rule and leave the observation as the discovery record (note the promotion in both). Add an `agent/log/` entry for the promotion.
...
**How to apply:** record an in-progress hypothesis or unratified method here with a `**Status:**` line, not as a rule. Promote it to `agent/rules/` only once it stabilizes, leaving the observation as the discovery record. Related: the agent-rules rule and the decision-log rule.
