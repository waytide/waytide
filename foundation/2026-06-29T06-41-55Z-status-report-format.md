...
3. **Agent-directory orientation** — a basic map of `agent/` (`rules/`, `log/`, `plans/`, `observations/`, `deferred/`, and any package-contributed directories such as `loops/`) and how to work with it: rules are enforceable conventions; the log is title-only ISO-8601-UTC decision entries; a rule typically pairs with a log entry; plans hold the task checklist; deferred holds parked work.
