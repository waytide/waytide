...
- **Resolution:** when the gating task is done, act on the item, then **delete the file** — the change itself lands in code or rules, and an `agent/log/` entry records that it was carried out. Deferred items are a queue, not a permanent record.
