...
- **Monorepo authoring, split distribution.** The canonical "Agent Norms" repo is the single source of truth, holding every package as a top-level directory with its own full history. Each `agent-norms-*` distribution repo is a *regenerated one-package view*, produced by `git subtree split --prefix=<package>` and pushed to its own repo in the org. Sync is one-directional (monorepo → solo repo), deterministic and incremental. You commit in the monorepo; a release is "re-split the changed packages and push." Chosen over polyrepo because the packages are tightly interrelated and the migration is heavily cross-package, so atomic cross-package commits and one place to read/grep/consolidate are worth the split step.
