...
- **Each package carries its own decision log.** A `<package>/log/` directory records decisions about that package's rules and travels with the package when a project installs it (landing at `agent/rules/<package>/log/`), as provenance — the open-source package ships its reasoning alongside its rules. This is distinct from the *project* decision log (the `foundation` convention: a consuming project's own `agent/log/` for its own decisions). A decision local to a package lives in that package's log; a decision that is genuinely cross-package or about the migration itself is a `foundation` decision (framework conventions) or, failing that, a transitional entry in the composite repo's top-level `agent/log/`.
