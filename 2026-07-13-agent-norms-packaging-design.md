...
Because the files are physically present and committed in the project, the session-start "read every file in `agent/rules/`" behavior works unchanged. Dependencies are declared per package (an `include: …` line, e.g. `design-by-efferent` needs `foundation`, `language`, `testing`) so a project knows the full set to add.
