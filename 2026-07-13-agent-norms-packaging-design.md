...
- **vocabulary** — the cross-cutting phrasing discipline: the `name-literally` and `no-slang` principle rules, plus one rule per general phrasing substitution (see Consolidations — no `substitutes` table). Holds only substitutions that are *not* specific to the design method or the test framework.
