...
constant name), the string starts with **"some"** — e.g. `control_value = "some string"`,
not `"a string"`. The word "some" marks it as an arbitrary placeholder value
whose specific content does not matter.

This parallels the constant-name convention, where example names are
`"SomeConstant"`, `"SomeOtherConstant"` (capitalized, because they are constant
names). A bare string *value* is lowercase: `"some string"`, `"some other string"`.
...
**How to apply:** Write `control_value = "some string"` (and `"some other
string"`, etc.) for placeholder string values. Reserve specific string content
