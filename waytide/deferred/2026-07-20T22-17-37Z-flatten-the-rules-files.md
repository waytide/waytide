...
"flatten" covers, then reshape `rules/` accordingly — and if it removes the
per-package directory, redesign the publish (split prefixes, `install-all.sh`,
CONTRIBUTING) in the same change, since those all key on `rules/<package>`. Related:
the agent-rules-convention, the just-completed grouping of packages under `rules/`,
and the CONTRIBUTING publish workflow.
