...
**Scope — method names only.** This governs **method/initializer names**, not prose or terminology. Prose deliberately *does* use prepositional phrasing where it carries meaning — e.g. inputs are described as **"sent to"** a method (the sent-to phrasing rule, in `language`). That is a description of a relationship, not an identifier; the preposition is load-bearing there and absent here. (The relation verb is the bare **"mediates"** — see the no-slang rule in `language` — so it is not an example of load-bearing prepositional prose.)
