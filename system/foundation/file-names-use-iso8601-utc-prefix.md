...
- The **working-state** directories: the running records under `logs/` — the decision log (`logs/log/`), `logs/loops/`, `logs/work-sessions/`, and `logs/migration/` — and the work-artifact directories `ideas/` and `implementations/`.
