...
**How to apply:** record an in-progress hypothesis or unratified method here with a `**Status:**` line, not as a rule. Promote it to `waytide/local/rules/` only once it stabilizes, leaving the observation as the discovery record. Related: the agent-rules rule and the decision-log rule.
