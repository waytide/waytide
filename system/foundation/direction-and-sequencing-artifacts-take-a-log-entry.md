...
**The entry names the record and what happened to it.** That is the record being written, or its
`**State:**` line reaching a word. What the record settles may follow, attributed to the record
rather than stated as the system's behavior.

**An entry never states an unbuilt direction in the indicative mood.** A direction artifact settles
a direction and builds nothing. *The check runs at the engineer's first prompt* says the system does
that, and while the record is `Open` or `Active` the system does not.
...
passive voice and indicative mood. Its commit-says-task-is-done-not-mark-done rule asks for the fact
rather than the clerical gesture. Each is written for a change to the software.

**A direction artifact is not a change to the software.** The fact about it is that it was recorded,
or that its state moved. Writing what it settles in that grammar states a fact that did not happen,
which is the one thing those rules never license.

**This reaches the commit subject the entry travels in**, which is the more-scanned surface. An
entry corrected in the log while its commit subject carries the same claim is half corrected.

**No wording template is fixed.** *X is recorded as an idea — Open because …* and *X is realized*
are forms already in use. What is required is the record named and the mood honest, and a template
would be the over-prescription the plans-no-code-samples rule warns against.
...
**The mood is required because the log is the artifact a reader finds holding nothing else.** Every
other record carries a `**State:**` line saying whether the thing exists. A log entry carries a
title. Where the title asserts what a record only settles, the reader has nothing to weigh it
against, and reads a proposal as the system's behavior.
...
Name the record in the entry, and say what happened to it. State what the record settles as the
record's, never as the system's behavior, and carry that into the commit subject the entry travels
in. Fix no template for the wording.
...
- the `git` package's subject-first-commit-messages and commit-says-task-is-done-not-mark-done rules
  — the grammar that is right for a change to the software and wrong for a record that settles one
- the ideas-convention — the record whose states an entry reports, and which this rule's name
  predates
