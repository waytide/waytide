...
- **`**State:**`** — the canonical state line, carrying one of the six state words. It sits above the goals, and every plan has one — a new plan is **Active**. See the plans-convention rule, which fixes the words.
...
**How to apply:** when writing a plan, carry the elements the work has, ordered as above where they appear. State each task as a behavior with a single checkbox. Add an element the work calls for rather than forcing it into one of these. Point **Source design** at the design being realized, and keep code out of the document. Give a new plan a `**State:**` line reading **Active**, above the goals, and change it as the plan reaches one of the other five words.
