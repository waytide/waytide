...
**Offer an `Explain` option.** It is not an escape and does not duplicate the free-text choice: the built-in escape is how an engineer **answers** outside the options. This is how they ask what the question **means** before answering at all. An engineer who does not understand the options cannot produce a better one. So the two serve opposite halves of the moment responsibility transfers to them.
