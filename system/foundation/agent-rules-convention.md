...
- **Rules are kept separate from work artifacts.** `waytide/system/` and `waytide/local/rules/` hold only rules. The project's working state lives in sibling directories under `waytide/` — `log/`, `deferred/`, `observations/`, `design/`, `plans/`, `sessions/`, `loops/`, `experiments/` — worked with as their own conventions describe, not read as binding rules at session start.
