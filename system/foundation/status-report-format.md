...
**The report reads the project's sequencing artifact, whichever the project's mode names it.** Under formal that is a **plan** in `waytide/local/plans/`; under creative an **intention** in `waytide/local/intention/` (see the a-project-works-in-formal-or-creative-mode rule). The report is described below in the formal mode's terms because formal is the default; read *plan* as *intention* and `plans/` as `intention/` in a creative project.
