...
- **Record format:** open with the **Question**, the hypothesis. The setup: - a **`**State:**` line** - the **upstream branch** it will merge into. `master` or another topic branch - the **experiment branch** - its **base** - the **working location** chosen at the start. Branch only, or branch and worktree with the worktree's path
