...
The word is used as a general quality of a design. It is judged **from the outside**. From the use site, not from the implementation. An implementation that only works by forcing an awkward use is not soluble, however tidy it reads internally.
...
- the name-literally-not-by-analogy rule — which uses solubility as its criterion -. For the term's role inside a specific method. The `design-by-efferent` package's vocabulary - where solubility is the target of generation and a known hinge
