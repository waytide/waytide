...
# and no vocabulary claims.
#
# Usage, from the project root:
#   waytide/system/language/report-unclaimed-words.sh [dictionary-path]
#
...
# prose uses it. A word already claimed in a vocabulary is not reported.
#
# It reads only. It writes nothing and it fetches nothing.
#
# Exit 0 when nothing is unclaimed. Exit 1 when something is. Exit 2 when the
...
# Every term any vocabulary declares, from a bold-led bullet or a table row.
...
# A refused word the prose uses and no vocabulary claims.
join -t "$(printf '\t')" /tmp/ete-prose.$$ /tmp/ete-refused.$$ \
  | sort -u > /tmp/ete-used-refused.$$

join -t "$(printf '\t')" -v 1 /tmp/ete-used-refused.$$ /tmp/ete-claimed.$$ \
  | sort -t "$(printf '\t')" -k1,1 > /tmp/ete-unclaimed.$$
...
join -t "$(printf '\t')" -v 1 /tmp/ete-unclaimed.$$ /tmp/ete-approved.$$ \
  | sort -t "$(printf '\t')" -k2,2nr -k1,1 > /tmp/ete-refused-outright.$$
join -t "$(printf '\t')" -o 1.1,1.2,1.3 /tmp/ete-unclaimed.$$ /tmp/ete-approved.$$ \
...
claimed=$(wc -l < /tmp/ete-claimed.$$ | tr -d ' ')

echo "Unclaimed words"
echo
echo "  Dictionary:   $dict"
echo "  Rule files:   $(echo "$rules" | wc -l | tr -d ' ')"
echo "  Vocabularies: $(echo "$vocabularies" | wc -l | tr -d ' '), declaring $claimed terms"
echo "  Distinct words in the prose: $words"
echo

if [ "$outright" -eq 0 ] && [ "$crossed" -eq 0 ]; then
  echo "Every word the dictionary refuses is claimed in a vocabulary."
  rm -f /tmp/ete-*.$$
  exit 0
fi

if [ "$outright" -gt 0 ]; then
  echo "Refused in every part of speech, and claimed by no vocabulary: $outright"
  echo "Each is a word to claim in a vocabulary, or prose to rewrite."
  echo
