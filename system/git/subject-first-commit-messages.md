...
**Why:** the log records the history of the software, and it is scanned rather than read, so leading with the changed thing rather than the engineer's action is both truer to what the log is for and easier to review. It is the Eventide house style, and it composes with the commit-wording rule — "Task 9 is done" already states the fact, software-first.

**How to apply:** name the software that changed, then state its new state in passive, indicative voice, subject leading. Never lead with a verb — not imperative and not past tense — because both put the engineer's work ahead of the software. Related: the commit-says-task-is-done-not-mark-done rule, the no-Claude-coauthor rule, and the run-suite-before-commit rule.
