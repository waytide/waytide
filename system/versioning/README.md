...
- **A version is semver, optionally prefixed by a product generation number**
  ...
  — nothing, nothing but read, or change their code. The leading segment, where a project carries one, answers *is this the same product*, which is a **declaration** rather than a reading of the change. Is never increased at a release.
...
- **The commit** that records a version change is a commit concern. Stays in the `git` package. `Package version is increased from 1.1.1 to 1.2.0`, in the subject-first rule.
- **The mechanics** — where the version is recorded. How the package is built and published. Belong to the technology the package is distributed with. For a Ruby gem that is
...
- **This package has no knowledge of any of that**, even though every package versioned under
  it today is a Ruby gem. What a version means does not change with the packaging technology. A rule that mixed the two would be unusable by a project that packages differently.
...
not an act but a meaning. Not *version*: a version is a particular value, where this package governs the practice of assigning and interpreting them. `waytide/version` would read as the version of Waytide itself rather than of the things made with it.
