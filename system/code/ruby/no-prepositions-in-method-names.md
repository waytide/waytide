...
Do not put a preposition in a method name (`import_from`, `define_in`, `set_to`, `copy_into`). A preposition in a method name points at the **argument** — but arguments are **inherent to methods**. Nothing in the name needs to announce them. The parameter already says "what follows is an argument". A preposition-word adds no information the call site doesn't already carry. Name the method for the action alone (`import`, `define`, `set`, `copy`).
