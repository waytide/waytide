...
**How to apply:** Give every optional parameter. Positional or keyword. A `nil` default in the signature. Assign its real default with `||=` at the top of the body. Note `flag ||= false` for a boolean default normalizes only `nil → false` (a `true` passes through). That is intended and keeps the body's defaulting uniform even when the default is falsy. Keep the assignments free of inlined method calls per the no-inline-method-call-arguments rule (constants and literals as the default are fine). Related: the no-inline-method-call-arguments rule.
...
Examples: - `Upload#call` forwards `verify` to `http_client.post` (which defaults it) in its delegating branch. Passing `verify` raw there — but coalesces `verify ||= false` in the branch that inspects the response itself.
