...
**How to apply:** An argument may otherwise be a method-call result. Assign it to a local named for the value, such as `response` or `result`. Then pass that local. This is the same impulse as the test convention. That convention binds the actuation result to an explaining variable before asserting on it. (Plain variables, constants, and literals as arguments are fine — the rule is about *method-call results*.) Related: the show-the-test/explaining-variable conventions.
