...
**How to apply:** when reporting that the unit is implemented and the test confirms, include the test file's content (and the run output) in the same turn, then hand over the solubility gate. Related: the first-turn / first-gate rule, the human-in-the-loop rule (wait at the actuation and at solubility), the show-feature-test-output rule (which sharpens this to the feature's full output at every implementation hinge), and the one-outcome-at-a-time rule.
