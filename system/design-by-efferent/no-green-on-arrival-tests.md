...
**How to apply:** an outcome's implementation hinge may be a no-op, meaning the test would be green-on-arrival. Drop the outcome rather than keeping the confirming test. The behavior may genuinely need locking to prevent a public-contract regression. That is a deliberate, separate decision the human makes. It is not the default, and it is not a DBE design step. **This is a design concern and does not reach coverage.** The activity may be preventing a regression in existing behavior rather than designing new behavior. Green-on-arrival is then inevitable and correct. See the "TDD designs, coverage prevents" rule.
