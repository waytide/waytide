...
**How to apply:** after writing an accepted implementation and running the suite, run the current feature's test files with verbose output. Include that output in the turn that reports the implementation. Then hand over the solubility gate. This sharpens the show-the-test-after-implementing rule — that rule shows the just-satisfied test. This one requires the **feature's full test output** at **every** implementation hinge.
