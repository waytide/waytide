...
plan" covers all of planning. **One of its two questions is answered:** the `plan` package was
folded into `foundation` on 2026-08-03, so the package-boundary question below is settled — there
is no `plan` package, and all three planning artifacts are governed by `foundation`. What remains
open is whether the **activity** wants a name, and the artifact-naming question the observation
records last. It records a structure the system has and does not describe. It
