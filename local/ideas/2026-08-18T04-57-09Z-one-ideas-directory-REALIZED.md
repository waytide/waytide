...
- **2026-08-18** — **`Lapsed` is a state word for an idea that lost its relevance over time.** It
  differs from **Abandoned** in who acted: abandoning is a decision, and lapsing is what happens
  when nobody decides and the work moves past the record. It belongs in the ideas convention when
  that is written, and is stated in the observations-convention meanwhile.
- **2026-08-18** — **`moot` is reserved in the `language` vocabulary** as the adjective for that
  condition — no longer bearing on anything because circumstances moved rather than because a
...
- ~~The migration of this project's own records.~~ **Resolved 2026-08-18** — done. Five
  aspirations and one intention moved to `waytide/local/ideas/`, each tagged `[aspiration]` or
  `[intention]`, with the `Source design` pointers reconciled. **Every consuming project's
  migration** is still open.
- ~~This project's `intuitive-mode` local rule is false and still binding.~~ **Resolved
  2026-08-18** — decommissioned. Its body is not kept, and a log entry names what it held.
