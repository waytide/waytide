...
**Does Waytide's prose conform to STE, what does conforming it cost, and does Waytide need an
implementation of its own?**

**Three questions, and the third was not asked at the start.** Whether the prose diverges is not in
doubt, since the STE experiment already recorded *rules Waytide breaks throughout, sentence length
above all*. What is in doubt is whether conforming the corpus is worth doing, in whole, in part, or
not at all. A survey that produced a list and no judgment would answer nothing.

**The third question is the one the work answered.** Conforming the terminology reached the limit of
what the standard covers, in the verbs a design method turns on and in the words it uses with its
own meanings. The answer is Eventide Technical English, with five categories of its own, two
overrides of the standard, and a stated disposition toward ordinary vocabulary. The experiment was
renamed for it on 2026-08-12.
...
- **`ete-declares-its-own-categories`** adds **Design method terms**, a noun
...
- **`ete-fixes-the-meaning-of-a-word-in-its-category`** says a word claimed in an ETE category takes
  the meaning its entry into the ETE gives it, whether the dictionary omits the word, refuses it, or approves it
...
**Six were declared and five survive.** The first two were *Design method terms* and *Engineering,
process, and method*, and the rule stated a line between them. Three more followed as terms called
for them: *Evented systems and autonomous components* for `projection`, *Agent artifacts* for
`working-state artifact`, and *Human-computer interaction* for `attended` and `unattended`.

**The line between the first two did not hold, and the assignments showed it.** `loop` went to one
and `cycle` to the other, when a cycle is one traversal of the loop. `mean-bias` went to one and
`solubility` to the other, when both name qualities the gates manage. The two names also both
carried the word *method*.

**They merged into `Engineering, design, process, and method`.** The merged name enumerates, which
is the idiom STE's own category names use. Eighteen entries moved. The rule's title changed with
the count, from *ETE declares a noun category and a verb category* to *ETE declares its own
categories*.

**Each category holds nouns and verbs.** The first declarations labelled each as one or the other,
which broke on the first word that was both. An entry names its own part of speech, since STE Rules
1.7 and 1.13 turn on which one a word is reserved as, and a category holding both kinds does not
make a word both.

## The vocabularies name their categories — 2026-08-13

**Sixty entries across the five vocabularies now name a category and state what the STE dictionary
says about the word.** Before this, the preambles made one blanket claim of category 19 for every
term in a file, which was false for every verb and wrong for the terms in categories 15 and 11.

**Where the reservations landed.** Most went to ETE engineering, design, process, and method. STE
category 11 took `engineer`. STE category 15 took `plain text data attribute line`. STE category 7
took `controls` and `precondition`. The other three ETE categories took one or two each.

**Nine entries state the first status and reserve nothing.** `prevent`, `wait`, `install packages`,
`effect`, `examine`, `start`, `name`, `activate`, and the citation-form row, which fixes how a
reference is written and reserves no word at all.

**`reserve` stands over `claim` for the act, on STE Rule 1.13 rather than on incumbency.** `claim`
names the act more accurately, since what happens is an assertion a reader can check. But Rule 1.13
forbids a technical verb used as a noun, `claim` has no separate noun form, and the corpus needs
*a claim* as an ordinary noun in many places. `reserve` has `reservation`.

**Silence in an entry now reads two ways.** A definition with no STE note names a term absent from
the STE dictionary and reserves it. A substitution's Say word with no note is ordinary vocabulary,
which is not reserved and not rewritten.

**That second default was forced by two words.** `required` and `justified` are Say words that
replace figures and name nothing in the subject field. The single default read their silence as a
reservation, so both were reserved in categories that did not fit. A third status would have
recorded them, which the ordinary-vocabulary rule forbids in its own words: *record no entry for
it*.

**The sequence for deciding a category exists and is written as a procedure nowhere.** Look the word
up. Ask what it names. Decide noun or verb. Try an STE category first. Fall to an ETE category only
where none holds. Those five steps are inferable from three rules and stated as a sequence in none
of them.

## Terms renamed and definitions corrected — 2026-08-13

**`solubility` is cognitive.** It was defined as how readily a unit dissolves into use, judged from
the use site. It is now how readily a unit dissolves and disappears smoothly into the reader's
cognition and understanding, with immediate comprehension as the ideal. The use-site framing
survives only in the DBE specialization, where solubility is the focus of the actuation hinge.

**`efferent` had its direction inverted.** The definition read *the view that conducts outward from
the unit toward its use*, which points from the thing being used. It now reads *the view that
conducts outward from the unit doing the using to the unit being used*.

**`outside in` is discontinued**, in all five places that carried it, because the phrase has been
misinterpreted in the industry. Each now says the interface is designed from the use site.

**`caller` is replaced in every sense.** An earlier ruling exempted the concrete party that invokes
a method, and that carve-out is removed. Sixteen uses in `code/ruby` take `efferent`, and `caller`
appears only in the `language` vocabulary, named as the common software-development patois.

**`origination escape` becomes `the free-text option`, and `origination` is discontinued.** The
compound was a metaphor stacked on a nominalization. The slogan *Provoke origination, not
mean-bias* becomes *Make the human produce the answer, not approve one*.

**`generation` is stated in full as `code generation` and defined.** It was used eighty times and
defined nowhere, while `product generation` was defined in `versioning` with an unrelated meaning.

**`recognizable-content line` becomes `plain text data attribute line`.** The form is used
throughout, in lines such as `**State:** Suspended`, and the old name was cited in three live places
against hundreds of lines written in the form.

**`owed` becomes `required`, and `required` is ordinary vocabulary.** The debt figure went into the
five preambles in the same pass as the rules that forbid figures. The replacement is not approved
either, and the dictionary's alternative does not serve: `NECESSARY (adj)` states a condition rather
than what a rule asks.

**An STE note says `absent from the STE dictionary` rather than bare `absent`.** Thirty-two notes
said only `absent`, which left a reader to carry the meaning from the preamble.

## What the check still cannot say — 2026-08-13

**`report-unreserved-words.sh` reports 396 refused words that no vocabulary reserves.** Most are
ordinary English that will never be reserved. The report cannot tell those apart from words nobody
has examined, and no check can, because the difference is a judgment about what a word names.

**It also matches only the dictionary's headword.** `require` is reported three times and
`required`, `requires`, `requirement`, and `requiring` are not, though the corpus uses those
forty-five times. Nothing about `justify` is reported at all, because every form of it is absent
from the STE dictionary and the check reports only refused words.

**And it does not check that a reserved entry names a category.** An entry written without one
passes silently, which is the failure most likely to happen when the work is done in a hurry.


## The scheme is written down and cited — 2026-08-13

**The rules that define ETE were cited from nothing that describes it.** All five vocabularies cited
the three ETE rules, and the three files that describe the `language` package did not. Those were
the two older `ete/` rules, written before ETE had categories, and the package README, which never
named ETE at all.

**The README gained a section for the directory.** All six rules are listed, and each of the three
ETE rules names the STE rule it contradicts. `Eventide Technical English` is spelled out on first
use with its abbreviation, which the package's own rule requires and which the README had never
needed before.

**The two older rules gained a paragraph each**, naming the three departures and the rule each
contradicts. The writing-rules table now marks its own STE Rule 1.3 row as overridden for a word
reserved in an ETE category, which is the one row in that table the corpus deliberately breaks. The
reserve instruction now says the entry names its category.

**The sequence for reserving a word is a rule.** It is `a-word-is-reserved-in-six-steps`: look the
word up, ask what it names, decide noun or verb, try an STE category, fall to an ETE category, write
the entry. Each step was already stated in one of three rules and the order was stated in none of
them, so a writer had to read all three and infer it.

**The rule states why the order is not arbitrary**, which is the part that could not be inferred.
Step 2 comes before step 3 because an ordinary word never reaches a part-of-speech decision. Step 4
comes before step 5 because an ETE category is a departure from the standard, and a departure is
taken only where the standard has nothing.

**The `ste/` directory is renamed `ete/`.** It holds Waytide's implementation rather than the
standard, and its own rules say so. Two of the six rule filenames keep `ste`, because they name the
standard rather than the directory.

**A seventh survivor of the reverted coordination pass was found and repaired**, in the
keeps-its-word rule's `Related:` list. One bullet had been split into two, leaving *which is the
authority this rule defers to* with no subject. Every one of the seven was found by reading a file
for an unrelated reason, thirteen days after the pass was reverted.


## The check is removed — 2026-08-13

**`report-unreserved-words.sh` is deleted.** It read the rule prose, compared it against the
ASD-STE100 dictionary and every vocabulary in reach, and reported the refused words that no
vocabulary reserved.

**What it could not do is what a reader would want from it.** Its report held 396 words. Most are
ordinary English that will never be reserved, and the report could not tell those apart from words
nobody had examined. The two states are identical in the corpus, since each is a word with no entry
that was never rewritten.

**Addressing that needed a record of what had been examined and left**, which the
ordinary-vocabulary-is-not-reserved-and-not-rewritten rule forbids in its own words. The three ways
out were a word list beside the script, a vocabulary entry with the rule amended, or accepting the
limit and reframing the report.

**The engineer removed the script instead.** Its stated reason is that it was becoming convoluted.

**The experiment's own evidence is against the check rather than for it.** Every finding here that
held came from reading. Every check that promised to replace reading failed: the `both` sweep, the
coordination pass reported clean, and the seven damaged sentences found one at a time while reading
a file for an unrelated reason.


## Affirmed — 2026-08-13

**The engineer declared the affirmation on 2026-08-13**, through the selection UI, and the record
carries the state on its canonical line.

**The three questions and their answers.**

*Does Waytide's prose conform to STE?* It did not. It now conforms on every mechanical class. 844
over-length sentences to zero, 137 over-length paragraphs to zero, 401 semicolons to zero, and none
of Waytide's own substitutions broken. It does not conform on the ordinary vocabulary, deliberately,
and a rule says so.

*What does conforming cost?* For the mechanical classes it cost 615 sentences composed one at a
time, 137 paragraphs broken, ten substitutions applied corpus-wide, three transformations reverted,
and seven measurement defects each found by reading. For the vocabulary it cost sixty entries, each
naming a category settled one word at a time.

*Does Waytide need an implementation of its own?* Yes. Eventide Technical English, with five
categories, two overrides of the standard, a rule leaving the ordinary vocabulary alone, and a rule
stating the sequence for reserving a word.

**The forecast stands as scored against the survey**, which is what it was written to predict. Items
4, 9, and 10 keep *Not scored* and *Open*, on the engineer's decision. The later sections carry what
was found.

**What the experiment leaves in the corpus.** Six rules in `system/language/ete/`. Sixty vocabulary
entries naming their categories, across five files. Ten corpus-wide substitutions. A local rule
saying a package is published on a release and never from an experiment.

**What it leaves undone, and states plainly.** Passive voice under STE Rule 3.6 was never measured.
The words the dictionary refuses in the ordinary vocabulary are unexamined one by one, and the check
that would have listed them is removed. Neither is a defect in what was built.

**The transferable findings, in the order they were learned.** A count of a corpus is not evidence
until a sample of what it counted has been read. A check written from the last failure does not find
the next one. A claim about an external standard is not evidence until the standard is read, and
here the standard sat in the repository the whole time.
