...
**Is journaling a good fit for Waytide?**

**Journaling here is a personal log written by the engineer**, or by more than one engineer. It is
not a record the agent writes. That is what separates it from everything `waytide/local/` holds
today.
...
journal is not a record of a decision, and it is not evidence. It is thinking, kept.

## Setup

- **Upstream branch:** `master`
- **Experiment branch:** `experiment/journaling`
...
1. **A journal will not fit under `waytide/local/`.** Everything there is the agent's work or the
...
2. **The strongest case for it is recall across sessions.** An agent that reads the engineer's
   journal knows what they were thinking, which no other artifact carries. *Moderate confidence.*
3. **The strongest case against it is that the agent would read it.** A journal read by an agent
...
4. **Waytide already covers most of what a journal would carry**, between the decision log and the
   work session record. What is left is the part the engineer would not want read. *Moderate
   confidence.*
5. **A journal will not need a rule.** Where it is personal, a convention naming a directory is
