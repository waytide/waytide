...
- **`**State:**`** — the canonical state line, carrying one of the six state words. It sits above the summary or premise, and every design has one — a new design is **Active**. See the design-convention rule, which fixes the words.
...
**How to apply:** when writing a design, open with a summary or premise and the motivation. Define the vocabulary it introduces. Develop the substance in named or numbered sections. Close with dated **Settled** resolutions and an **Out of Scope / Deferred** section. Give a new design a `**State:**` line reading **Active**, above the summary, and change it as the direction reaches one of the other five words.
